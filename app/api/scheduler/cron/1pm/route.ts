...
export async function GET(req: NextRequest) {
  ...
  const scheduledFor = getScheduledFor(13)
  ...
  for (const s of settings) {
    try {
      ...
      await generateAndSchedule(s.member_id, 'prize', scheduledFor, baseUrl, avatarSettings, platforms)
      postsScheduled++
    } catch (err) {
      console.error(`Cron 1pm failed for ${s.member_id}:`, err)
      errors++
    }
  }
