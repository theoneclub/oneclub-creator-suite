// Server-side Apify client (Deno Edge Function runtime). Called only with
// the service token below — never exposed to the browser.
const APIFY_BASE = 'https://api.apify.com/v2'

const ACTORS = {
  instagramProfile: Deno.env.get('APIFY_ACTOR_INSTAGRAM_PROFILE') || 'apify/instagram-profile-scraper',
  instagramHashtag: Deno.env.get('APIFY_ACTOR_INSTAGRAM_HASHTAG') || 'apify/instagram-hashtag-scraper',
  tiktokProfile: Deno.env.get('APIFY_ACTOR_TIKTOK_PROFILE') || 'clockworks/tiktok-profile-scraper',
  tiktokSearch: Deno.env.get('APIFY_ACTOR_TIKTOK_SEARCH') || 'clockworks/tiktok-scraper',
}
...
export async function searchInstagramByHashtags(hashtags: string[], limit: number): Promise<ScrapedPost[]> {
  ...
  const items = await runActorSync(ACTORS.instagramHashtag, { hashtags: cleanTags, resultsLimit: limit })
...
export async function searchTiktokByKeywords(keywords: string[], limit: number): Promise<ScrapedPost[]> {
  const items = await runActorSync(ACTORS.tiktokSearch, { searchQueries: keywords, resultsPerPage: limit })
