...
// ConvertKit's API is built around forms/sequences/subscriber tagging, not
// arbitrary one-off transactional sends of custom body content — there is
// no equivalent "send this exact message to this address" endpoint. Same
// for a generic "Other" API key, whose shape we don't know. Both fall back
// to copy-paste in the UI rather than silently no-op sending.
async function unsupportedSend(provider: EmailProvider): Promise<void> {
  throw new Error(`SEND_NOT_SUPPORTED: ${provider}`)
}
...
export async function sendTransactionalEmail(input: SendEmailInput): Promise<void> {
  switch (input.provider) {
    ...
    case 'convertkit':
    case 'other':
      return unsupportedSend(input.provider)
  }
}
